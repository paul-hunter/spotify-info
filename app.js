...
const client_id = '5453a49811cd4b9686761335b0d9adfb';
const client_secret = process.env.SPOTIFY_CLIENT_SECRET;

// just local for now while getting up and running need to change for heroku
...
app.get('/dbpost', (req, res) => {
    mongo.connect(mongo_url, (err, db) => {
        if (err) {
          console.error(err);
          return;
        }
        const collection = db.db('spotify_info').collection('popularity');
        collection.insertOne({name: 'Paul'}, (err, result) => {
            if (err) throw err;
        });
        db.close();
    });
